...
class Workspace(UUIDAuditMixin, Base):
    """A workspace represents a Slack workspace that has installed the app."""

    __tablename__ = "workspaces"

    slack_team_id: Mapped[str] = mapped_column(String(255), unique=True, nullable=False)
    slack_team_name: Mapped[str] = mapped_column(String(255), nullable=False)
    install_metadata: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
...
class User(UUIDAuditMixin, Base):
    """A user represents a Slack user in a workspace."""

    __tablename__ = "users"
    __table_args__ = (UniqueConstraint("slack_user_id", "workspace_id", name="uq_users_workspace_slack_user"),)
    ...
    slack_user_email: Mapped[str | None] = mapped_column(String(255), nullable=True)
...
class Incident(UUIDAuditMixin, Base):
    ...
    status: Mapped[str] = mapped_column(String(32), nullable=False, default=DEFAULT_INCIDENT_STATUS)
    severity: Mapped[str] = mapped_column(String(16), nullable=False, default=DEFAULT_INCIDENT_SEVERITY)
    mode: Mapped[str] = mapped_column(String(16), nullable=False, default=DEFAULT_INCIDENT_MODE)
    
    started_at: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
    detected_at: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
    mitigated_at: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
    ...
    workspace_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False, index=True)
    slack_channel_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)

    workspace: Mapped[Workspace] = relationship(back_populates="incidents")
    settings: Mapped[IncidentSettings | None] = relationship(back_populates="incident", uselist=False, cascade="all, delete-orphan")
    
class IncidentSettings(TimestampMixin, Base):
    """Settings for an incident."""
    ...
    trigger_thresholds: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
    post_budget: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
    active_refresh_interval: Mapped[int | None] = mapped_column(Integer, nullable=True)
